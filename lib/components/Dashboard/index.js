      ...
      <BodyClass className="full-height theme-normal dashboard">
        <Grid fluid={true} style={{height: '100%', position: 'relative'}}>
          <Col
            ...
            <div className={childSidebar ? 'collapse' : ''}>
              <h4>My Networks</h4>
              {0 !== networkCount && <div>
                <Nav>
                  {myNetworks.map( (net) => {
                    let active = net.key == nid;
                    console.log('active', net.key, nid, active);

                    return <LinkContainer to={`/dashboard/network/${net.key}`} key={net.key} active={active}>
                ...
                {_.map(networkByEnt, (networks, parent) =>
                  <div key={`networks-${parent}`}>
                    ...
                    <Nav>
                      {networks.map( (net) => {
                        let active = net.key == nid;
                        <LinkContainer to={`/dashboard/network/${net.key}`} key={net.key} active={active}>
                          <NavItem>{net.name || net.key}</NavItem>
                        </LinkContainer>
                      })}
                    </Nav>
