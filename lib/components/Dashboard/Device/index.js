...
export class Device extends React.Component {
  ...
  componentWillReceiveProps(nextProps) {
    let
      ...
      nextNid = nextProps.params.nid,
      nextKey = nextProps.params.key

    if (nextNid !== nid)
      NetworkService.fetch(nextNid)

    if (nextKey !== key || nextNid !== nid)
      DeviceService.fetch(nextNid, nextKey)
  }
  ...
  componentWillUpdate(nextProps, nextState) {
    if (!nextState.device) {
      ...
      DeviceService.fetch(nextProps.params.nid, nextProps.params.key)
    }
  }
